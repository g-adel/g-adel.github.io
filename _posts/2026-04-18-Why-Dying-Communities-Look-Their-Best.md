...
Luckily, one yearly survey question regarding when the user first started using the language can help uncover what's going on. The visualization they provided however is simply painful to read and can be quite difficult to see how the responder demographic looks like.[^1] I took the liberty of transforming and replotting the dates to be relative to the year of the survey:

![Users relative to year of reply](/assets/An-End-of-Life-Rally/julia-relative-year.png)

A much starker picture appears when looking at the evolution of this curve. You can think of it as the demographic distribution of the age (years of experience with Julia) of survey respondents each year. Early on (2019 up to 2022) there was a healthy curve sloping down, which means that there was a continuous inflow of new users with the most just being a year prior. However, gradually up to 2023 and then sharply in 2024/25, the fraction of newcoming users is much lower than before (going from over 20% to less than 10%) and is no longer the peak of the age demographic. [^2]

With that picture in mind, reexamining the previous promising statistics from the survey paradoxically reveals instead signs of community decline. When observing such a rise in positive metrics such as user satisfaction and average contributions, it is naturally tempting to assume the ecosystem is improving. However, what is likely actually happening is that the influx of newcomers has dropped off, causing the demographics to skew heavily toward veteran community members who have been committed members for a long time. [^3]
...
[^2]: Alternatively, a declining fraction of newcomers could occur despite a stable influx if the total community size continues to grow. However, such a phenomenon cannot explain by itself the magnitude of newcomers' decline.

[^3]: A counter-argument to this finding is that the userbase is simply maturing. Given that Julia only became available about 15 years ago, the ecosystem may just be abandoning its initial hype cycle and is in the process of settling into a more stable and sustainable community.